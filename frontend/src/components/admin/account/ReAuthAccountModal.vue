...
watch(
  ...
  (newVal) => {
    ...
    if (isKiro.value) {
      ...
      // 「从 Kiro IDE 导入」账号来源:按现有凭证的 provider/auth_method 自动定位。
      kiroImportProvider.value = resolveKiroImportProvider(provider, authMethod)
    }
  }
)
...
const resolveKiroImportProvider = (
  ...
  if (authMethod.trim().toLowerCase() === 'external_idp') {
    return 'ExternalIdp'
  }
