      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('expense', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="dollar" size="sm" />
              {{ t('admin.costCenter.addExpense') }}
            </button>
          </template>
        </div>
      </div>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'expense'])
const { t } = useI18n()
