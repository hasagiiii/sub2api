...
// VideoModelIcon 视频菜单图标（heroicons/outline film）。
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25zM3.75 8.25h16.5m-16.5 7.5h16.5M8.25 3.75v16.5m7.5-16.5v16.5'
        })
      ]
    )
}

// MaterialsIcon 素材库菜单图标（heroicons/outline photo）。
const MaterialsIcon = {
  render: () =>
    h(
      'svg',
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/purchase', label: purchaseNavLabel.value, icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: flagPayment, showDot: flagPurchasePromoDot },
...
function handleBrandClick() {
  // Brand links navigate to "/" via router-link; mirror handleMenuItemClick's
  // mobile-drawer close so tapping the logo/title on mobile dismisses the drawer.
  if (mobileOpen.value) {
    setTimeout(() => {
      appStore.setMobileOpen(false)
    }, 150)
