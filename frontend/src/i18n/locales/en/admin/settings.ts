    ...
    settings: {
      ...
      features: {
        ...
        company: {
          ...
          description: 'Manage company applications, IAM, and rollout-readiness switches. Configuration-file values are used until settings are saved.',
          applicationsEnabled: 'Enable company applications',
          applicationsEnabledHint: 'Allow eligible personal root accounts to submit company upgrade applications.',
          iamEnabled: 'Enable company IAM',
          iamEnabledHint: 'Allow company owners to create IAM members and IAM users to sign in.',
          publicIdsFinalized: 'Public IDs are ready',
          publicIdsFinalizedHint: 'Enable only after the public-ID backfill and final constraints are complete.',
          billingIntegrationEnabled: 'Company billing is ready',
          billingIntegrationEnabledHint: 'Enable only after all billing, refund, and reconciliation paths pass acceptance.',
          ...
          upgradeFee: 'Company upgrade fee (USD)',
          upgradeFeeHint: 'Applies only to newly submitted applications; existing applications keep their original fee snapshot.',
          upgradeFeeInvalid: 'The company upgrade fee must be greater than 0.',
          documentationURL: 'Company documentation URL',
          documentationURLHint: 'Optional. Shows a help icon beside the company console menu and opens this HTTP(S) URL in a new window.',
          documentationURLInvalid: 'The company documentation URL must be a valid HTTP(S) URL.',
        },
        affiliate: {
      ...
      security: {
        ...
        sessionBindingHint: 'Bind login sessions to the client IP and User-Agent. Any change immediately invalidates the session and forces re-login, raising the bar for stolen-credential reuse. Deployments behind a CDN/CF must also configure trusted_proxies correctly, otherwise real users will be incorrectly kicked out.',
        auditRetention: 'Audit Log Retention (days)',
        trustedProxies: {
          title: 'Trusted Proxies (Dynamic Fetch)',
          hint: 'Periodically fetch CDN / reverse proxy IP ranges from remote URLs and merge into Gin trusted_proxies; required for X-Forwarded-For / CF-Connecting-IP to be trusted. Merge order: config.yaml static + manual fixed + dynamic fetch.',
          staticLabel: 'Static list (config.yaml)',
          staticHint: 'From server.trusted_proxies, {count} entries total; requires restart to change.',
          staticShow: 'Show entries',
          extraLabel: 'Manually pinned CIDR / IP',
          extraHint: 'One CIDR or single IP per line (IPs auto-appended /32 or /128). Ideal for a fixed upstream nginx reverse proxy or a private LB.',
          extraPlaceholder: '192.168.1.0/24\n10.0.0.5',
          sourcesLabel: 'Fetch sources',
          addSource: '+ Add source',
          removeSource: 'Remove',
          namePlaceholder: 'Display name (e.g. Cloudflare IPv4)',
          urlPlaceholder: 'https://www.cloudflare.com/ips-v4',
          intervalLabel: 'Interval (s)',
          timeoutLabel: 'Timeout (s)',
          statusSuccess: 'Last success {at}: {count} entries',
          statusPending: 'Not yet fetched',
          noSources: 'No fetch sources yet; click "+ Add source" to configure one.',
        },
      ...
      gatewayForwarding: {
        ...
        codexTicketEnabledDesc: "When off, the gateway neither harvests nor injects x-codex-turn-state and forwards traffic as usual. When on, it harvests tickets in the background and overwrites that header on production requests.",
        codexTicketHarvestProxy: "292 harvest proxy",
        codexTicketHarvestProxyDesc: "Used only for minting 292 tickets when the ticket feature is enabled. Changes apply to subsequent probes without a restart. Production traffic still uses each account's residential proxy. Paste a full HTTP or SOCKS5h proxy URL including username and password.",
