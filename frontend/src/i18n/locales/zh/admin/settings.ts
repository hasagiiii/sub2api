export default {
    settings: {
      ...
      features: {
        ...
        company: {
          title: '企业功能',
          description: '管理企业申请、IAM 和上线就绪开关。未保存设置时使用配置文件中的默认值。',
          applicationsEnabled: '启用企业升级申请',
          applicationsEnabledHint: '开启后，符合条件的个人主账号可以提交企业升级申请。',
          iamEnabled: '启用企业 IAM',
          iamEnabledHint: '开启后，企业管理员可以创建 IAM 成员，IAM 用户可以登录。',
          publicIdsFinalized: '公共 ID 已就绪',
          publicIdsFinalizedHint: '仅在公共 ID 回填与最终约束已完成后开启。',
          billingIntegrationEnabled: '企业计费链路已就绪',
          billingIntegrationEnabledHint: '仅在所有计费、退款和对账链路验收完成后开启。',
          chargeEnabled: '企业升级收费',
          chargeEnabledHint: '开启后，提交企业升级申请将按规则冻结资金/收取升级费；关闭后申请不冻结、不扣费（费用快照记为 0）。',
          upgradeFee: '企业升级费用（USD）',
          upgradeFeeHint: '仅影响之后新提交的申请；已提交申请继续使用原费用快照。',
          upgradeFeeInvalid: '企业升级费用必须大于 0。',
          documentationURL: '企业说明文档地址',
          documentationURLHint: '可选。填写后，企业控制台菜单旁显示问号，并在新窗口打开该 HTTP(S) 地址。',
          documentationURLInvalid: '企业说明文档地址必须是有效的 HTTP(S) 地址。',
        },
        affiliate: {
          title: '邀请返利',
      ...
      security: {
        ...
        sessionBindingHint: '将登录会话与客户端 IP 和 User-Agent 绑定，任一变化即强制该会话失效并需重新登录（提升被盗凭证的利用门槛）。前置有 CDN/CF 的部署需要同时配置好 trusted_proxies，否则会误伤真实用户。',
        auditRetention: '操作日志保留天数',
        trustedProxies: {
          title: '可信代理动态拉取',
          hint: '从远端 URL 定期拉取 CDN/反向代理 IP 段，合并到 Gin trusted_proxies；开启后 X-Forwarded-For / CF-Connecting-IP 才能被信任。合并顺序：config.yaml 静态 + 手动固定 + 动态拉取。',
          staticLabel: '静态列表（config.yaml）',
          staticHint: '来自 server.trusted_proxies，共 {count} 条；修改需重启后端。',
          staticShow: '查看条目',
          extraLabel: '手动固定 CIDR / IP',
          extraHint: '每行一个 CIDR 或单个 IP（IP 自动补 /32 或 /128）。适合前置固定 nginx 反代或私网负载均衡。',
          extraPlaceholder: '192.168.1.0/24\n10.0.0.5',
          sourcesLabel: '拉取源列表',
          addSource: '+ 添加源',
          removeSource: '删除',
          namePlaceholder: '展示名（如 Cloudflare IPv4）',
          urlPlaceholder: 'https://www.cloudflare.com/ips-v4',
          intervalLabel: '间隔（秒）',
          timeoutLabel: '超时（秒）',
          statusSuccess: '上次成功 {at}：共 {count} 条',
          statusPending: '尚未拉取',
          noSources: '暂无拉取源，可点击『+ 添加源』手动录入。',
        },
      ...
      gatewayForwarding: {
        ...
        codexTicketEnabledDesc: '关闭后不打票、不注入 x-codex-turn-state，按原链路转发。开启后后台打票，并在业务请求中覆盖该头。',
        codexTicketHarvestProxy: '292 打票代理',
        codexTicketHarvestProxyDesc: '仅在门票功能开启时用于打票，保存后后续探测会使用新代理，无需重启。日常业务仍走账号自己的住宅代理。填写完整代理 URL（http 或 socks5h，含用户名和密码）。',
