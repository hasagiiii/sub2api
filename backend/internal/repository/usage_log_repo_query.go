...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	if imageURLsJSON.Valid {
		log.ImageURLs = unmarshalStringSlice([]byte(imageURLsJSON.String))
	}
	if cosURLsJSON.Valid {
		log.CosURLs = unmarshalStringSlice([]byte(cosURLsJSON.String))
	}
	...
	if organizationID.Valid {
		log.OrganizationID = &organizationID.Int64
	}
	if payerUserID.Valid {
		log.PayerUserID = &payerUserID.Int64
	}
	if balanceSource.Valid {
		log.BalanceSource = &balanceSource.String
	}
	if authzGeneration.Valid {
		log.AuthzGeneration = &authzGeneration.Int64
	}
	if sessionID.Valid {
	...
	if len(requestParametersJSON) > 0 {
		...
		if err := json.Unmarshal(requestParametersJSON, &requestParameters); err == nil {
			log.RequestParameters = requestParameters
		}
	}
