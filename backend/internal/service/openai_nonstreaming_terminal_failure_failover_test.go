...
func TestNonStreamingSSEToJSON_CapacityFailedEventFailsOver(t *testing.T) {
	...
	result, err := svc.handleSSEToJSON(context.Background(), newNonStreamingSSEResponse(), c, newNonStreamingFailoverAccount(), body, "model", "model", nil, "")
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := svc.handleSSEToJSON(context.Background(), newNonStreamingSSEResponse(), c,
				newNonStreamingFailoverAccount(), sseTerminalBody("response.failed", tc.data), "model", "model", nil, "")
...
func TestNonStreamingSSEToJSON_BareErrorEventUsesConservativeClassifier(t *testing.T) {
	t.Run("non_transient_stays_protocol_error", func(t *testing.T) {
		...
		result, err := svc.handleSSEToJSON(context.Background(), newNonStreamingSSEResponse(), c,
			newNonStreamingFailoverAccount(), sseTerminalBody("error", data), "model", "model", nil, "")
...
func TestNonStreamingPassthroughSSEToJSON_CapacityFailedEventFailsOver(t *testing.T) {
	...
	result, err := svc.handlePassthroughSSEToJSON(context.Background(), newNonStreamingSSEResponse(), c, newNonStreamingFailoverAccount(), body, "model", "model", nil, "")
	...
	for _, data := range payloads {
		t.Run(data[:min(len(data), 60)], func(t *testing.T) {
			...
			_, err := svc.handleSSEToJSON(context.Background(), newNonStreamingSSEResponse(), c,
				newNonStreamingFailoverAccount(), sseTerminalBody("response.failed", data), "model", "model", nil, "")
