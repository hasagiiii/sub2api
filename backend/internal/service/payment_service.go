...
// OrganizationSubscriptionPlanFulfiller is implemented by services that can
// preserve a paid package's shared quota semantics for enterprise rows.
type OrganizationSubscriptionPlanFulfiller interface {
	FulfillOrganizationSubscriptionPlanOrder(ctx context.Context, orgID, groupID, planID int64, validityDays int, orderID int64) error
}
