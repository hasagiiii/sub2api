...
// recordUsageOpts carries local billing fallbacks without changing public callers.
...
func shouldUseKiroConservativeBillingFallback(result *ForwardResult, _ string, opts ...*recordUsageOpts) bool {
	return result != nil && len(opts) > 0 && opts[0] != nil && opts[0].IsKiroAccount
}
...
func (s *GatewayService) calculateImageCost(
	...
	if apiKeyHasConfiguredImagePrice(apiKey, sizeTier) {
		return s.billingService.CalculateImageCost(billingModel, billingSize, result.ImageCount, groupConfig, multiplier)
	}
