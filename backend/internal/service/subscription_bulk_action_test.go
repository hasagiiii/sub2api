...
func (r *bulkActionSubscriptionRepo) GetByUserIDAndPlanID(_ context.Context, userID, planID int64) (*UserSubscription, error) {
	for _, sub := range r.subscriptions {
		if sub.UserID != userID || sub.PlanID == nil || *sub.PlanID != planID || sub.DeletedAt != nil {
			continue
		}
		copy := *sub
		return &copy, nil
...
func (r *bulkActionSubscriptionRepo) GetManualByUserIDAndGroupID(_ context.Context, userID, groupID int64) (*UserSubscription, error) {
	for _, sub := range r.subscriptions {
		if sub.UserID != userID || sub.GroupID != groupID || sub.PlanID != nil || sub.DeletedAt != nil {
			continue
		}
		copy := *sub
		return &copy, nil
