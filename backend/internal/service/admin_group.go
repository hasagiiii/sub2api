...
func compositeDefaultModelsListCandidateIDs() []string {
	...
	ids := make([]string, 0)
	for _, platform := range []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformLeonardo} {
...
// normalizeLimit 将 nil、负数转换为 nil（表示无限制）；0 保留为 0。
// 说明：与上游保持一致——0 表示"无限制"是在读取侧（Group.HasDailyLimit /
// OrgSubscriptionRuntime.CheckAllLimits）通过 *limit > 0 判定实现的，写入侧
// 保留原始值（包括 0），不做隐式归并。
