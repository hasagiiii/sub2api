...
func TestBatchImagePublicService_SelectAccountPriority(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			provider, account, err := svc.selectProviderAndAccount(context.Background(), &BatchImageOwner{UserID: 11, APIKeyID: 22}, BatchImageProviderGeminiAPI, "gemini-2.5-flash-image")
