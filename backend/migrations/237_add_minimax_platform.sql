...
ALTER TABLE composite_model_routes
    ...
    CHECK (target_platform IN ('anthropic','openai','gemini','antigravity','kiro','grok','fal','leonardo','atlascloud','apiz','higgsfield','kimi','zhipu','deepseek','minimax','bytedance'));
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('minimax' IN monitor_constraint_def) = 0 THEN
        ...
        ALTER TABLE channel_monitors
            ...
            CHECK (provider IN ('anthropic','openai','gemini','antigravity','kiro','grok','fal','leonardo','atlascloud','apiz','higgsfield','kimi','zhipu','deepseek','minimax','bytedance'));
    END IF;
